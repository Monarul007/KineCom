...
class PosSupplierController extends Controller
{
    ...
    public function edit(Request $request){
        $id = $request->id;
        $get_data = DB::table('suppliers')->select('suppliers.name','suppliers.phone','suppliers.address','suppliers.email')->where('id',$id)->first();
    ...
    public function updateSupp(Request $request){
        ...
        DB::table('suppliers')->where(['id'=>$id])->update(['name'=>$name,'phone'=>$phone,'address'=>$address,'email'=>$email]);
    ...
    public function suppDetails(Request $request){
        $id = $request->id;
        $get_data = DB::table('suppliers')->select('suppliers.name','suppliers.phone','suppliers.address')->where('id', $id)->first();
    ...
    public function getSupplier($id){
        ...
        $total_purchase = DB::table('purchase_primary')->where('sid',$id)->sum('amount');
        $purchase = DB::table('purchase_primary')->where('sid',$id)->get();
        $fromPurchase = DB::table('purchase_primary')->where('sid',$id)->sum('payment');
        $fromPayment = DB::table('payment_invoice')->where('cid',"sid ".$id)->sum('amount');
    ...
    public function filter_data(Request $request){
        if(request()->ajax()){
            ...
            if(!empty($request->from_date)){
                $data = DB::table('acc_transactions')->where('sort_by',$sid)->whereBetween('date', array($request->from_date, $request->to_date))->get()->toArray();
            }else{
                $data = DB::table('acc_transactions')->where('sort_by',$sid)->get()->toArray();
            }
    ...
    public function purchaseinvoice($id){
        $invoiceno = $id;
        $get_supplier = DB::table('purchase_primary')->where('pur_inv', $id)->first();
